...
export class MultiParentNodeSelectionService extends CommonTreeSelectionService implements TreeSelectionInterface {
  constructor(
  ...
  isDisabled(node: StandardFlatNodeModel): boolean {
    if (this.checklistSelection.isEmpty() || !this.checklistSelection.isSelected(node)) {
      return false;
    }

    return this.checkSomeParentSelected(node, this.checklistSelection);
  }
